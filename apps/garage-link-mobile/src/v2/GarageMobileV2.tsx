import { documentHtml, quoteHtml, documentLineAmountLabel, documentUnitPriceLabel } from './documentHtml';
...
import { Fragment, useEffect, useEffectEvent, useState, useRef } from 'react';
...
export default function GarageMobileV2({ store, onLogout, onStore }: { store: Store; onLogout: () => void; onStore: () => void }) {
  ...
  return <Shell title={title} tab={route.tab} onTab={tab} onBack={route.kind === 'tab' ? onStore : back} onLogout={onLogout} busy={busy} error={error}>
    ...
    {route.kind === 'quoteDetail' && quoteView && <Card title={quoteView.quoteNo || '見積'}><Info label="顧客" value={quoteView.customerName} />{quoteView.items.map((item,index) => <Fragment key={item.id || index}><Info label={`${item.name}（${documentLineAmountLabel(quoteView.taxDisplayMode,item.taxCategory)}）`} value={yen(item.amount ?? item.quantity * item.unitPrice)} /><Info label={`数量・単価（${documentUnitPriceLabel(quoteView.taxDisplayMode,item.taxCategory)}）`} value={`${item.quantity ?? '未設定'} ${item.unit || ''} × ${item.unitPrice == null ? '未設定' : yen(item.unitPrice)}`} /></Fragment>)}<Info label={quoteView.taxDisplayMode ? '小計（税抜・非課税を含む）' : '小計（保存時）'} value={yen(quoteView.subtotalAmount ?? 0)} /><Info label={quoteView.taxDisplayMode ? '値引き（税抜相当）' : '値引き（保存時）'} value={yen(quoteView.discountAmount ?? 0)} /><Info label="消費税" value={yen(quoteView.taxAmount ?? 0)} /><Info label="下取り" value={yen(quoteView.tradeInAmount ?? 0)} /><Info label="合計" value={yen(quoteView.totalAmount ?? 0)} /><Button title="PDFを保存・共有" onPress={() => void shareQuote(quoteView)} /><Button title="印刷" secondary onPress={() => void printQuote(quoteView)} /><Button title="請求を作る" secondary onPress={() => { setFormId(clientId()); push({kind:'invoice',tab:route.tab,quoteId:quoteView.id}); }} disabled={!canFinance} /></Card>}
    ...
    {route.kind === 'invoiceDetail' && invoiceView && <Card title={str(invoiceView,'invoice_no') || '請求'}><Info label="顧客" value={str(invoiceView,'customer_name')} />{invoiceItems.map((item,index) => <Fragment key={str(item,'id') || index}><Info label={`${str(item,'name')}（${documentLineAmountLabel(invoiceView.tax_display_mode,item.tax_category)}）`} value={yen(num(item,'amount'))} /><Info label={`数量・単価（${documentUnitPriceLabel(invoiceView.tax_display_mode,item.tax_category)}）`} value={`${item.quantity == null ? '未設定' : num(item,'quantity')} ${str(item,'unit')} × ${item.unit_price == null ? '未設定' : yen(num(item,'unit_price'))}`} /></Fragment>)}<Info label={documentLineAmountLabel(invoiceView.tax_display_mode,'taxable') === '税抜' ? '小計（税抜・非課税を含む）' : '小計（保存時）'} value={yen(num(invoiceView,'subtotal_amount'))} /><Info label={documentLineAmountLabel(invoiceView.tax_display_mode,'taxable') === '税抜' ? '値引き（税抜相当）' : '値引き（保存時）'} value={yen(num(invoiceView,'discount_amount'))} /><Info label="消費税" value={yen(num(invoiceView,'tax_amount'))} /><Info label="下取り" value={yen(num(invoiceView,'trade_in_amount'))} /><Info label="支払済額" value={yen(num(invoiceView,'paid_amount'))} /><Info label="請求額" value={yen(num(invoiceView,'total_amount'))} /><Info label="未入金" value={yen(num(invoiceView,'unpaid_amount'))} /><Info label="状態" value={str(invoiceView,'issue_status')} /><Button title="請求を発行" onPress={() => void issueInvoice(invoiceView.id)} disabled={!canFinance || str(invoiceView,'issue_status') === 'issued'} /><Button title="PDFを保存・共有" secondary onPress={() => void shareInvoice(invoiceView)} /><Button title="入金を登録" secondary onPress={() => { setDraft({paymentAmount:String(num(invoiceView,'unpaid_amount'))}); setOperationId(operationKey()); push({kind:'payment',tab:route.tab,invoiceId:invoiceView.id}); }} disabled={!canFinance || str(invoiceView,'issue_status') !== 'issued' || num(invoiceView,'unpaid_amount') <= 0} /></Card>}
