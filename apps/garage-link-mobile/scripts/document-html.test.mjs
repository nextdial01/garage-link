...
import {documentHtml,documentLineAmountLabel,documentUnitPriceLabel} from '../src/v2/documentHtml.ts';
...
test('detail labels distinguish persisted net amounts, snapshot input prices and non-taxable lines',()=>{
 for (const mode of ['included','excluded',null,undefined,'legacy']) {
  const known=mode==='included'||mode==='excluded';
  assert.equal(documentLineAmountLabel(mode,'taxable'),known?'税抜':'保存時');
  assert.equal(documentUnitPriceLabel(mode,'taxable'),mode==='included'?'税込':mode==='excluded'?'税抜':'保存時');
  for (const [category,label] of [['exempt','非課税'],['out_of_scope','税対象外']]) {
   assert.equal(documentLineAmountLabel(mode,category),label);
   assert.equal(documentUnitPriceLabel(mode,category),label);
  }
 }
});
