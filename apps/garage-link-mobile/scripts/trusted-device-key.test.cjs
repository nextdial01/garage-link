...
const secureStore = {
  ...
  getItemAsync: async (key) => { calls.push(['get', key]); return 'a'.repeat(64); },
  setItemAsync: async (key, value, options) => { calls.push(['set', key, value, options]); },
...
(async () => {
  ...
  assert.deepEqual(keys, ['garage-link.trusted-device-token', 'garage-link.trusted-device-token', 'garage-link.trusted-device-token']);
