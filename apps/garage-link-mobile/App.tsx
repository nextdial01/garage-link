...
function GarageMobileApp() {
  ...
  if (page === 'maintenance') return <Screen title="整備・車検" onStore={() => setPage('stores')} nav={nav} error={null}><ListState loading={loading} error={error} onRetry={() => void openMaintenance()}><FlatList data={jobs} keyExtractor={(item) => item.id} contentContainerStyle={styles.list} refreshing={loading} onRefresh={() => void openMaintenance()} renderItem={({ item }) => <EntityRow title={item.job_no || '整備案件'} subtitle={(item.job_type || '種別未設定') + ' · 納車予定 ' + date(item.scheduled_delivery_at)} right={<StatusChip value={item.status} />} onPress={() => void openJob(item)} />} ListEmptyComponent={<EmptyState title="整備案件がありません" description="今日の作業予定はここに表示されます。" />} /></ListState></Screen>;
  ...
  if (page === 'customers') return <Screen title="顧客" onStore={() => setPage('stores')} nav={nav} error={null}><Search placeholder="顧客名・電話番号で検索" value={search} onChange={setSearch} onSearch={() => void openCustomers()} /><ListState loading={loading} error={error} onRetry={() => void openCustomers()}><FlatList data={customers} keyExtractor={(item) => item.id} contentContainerStyle={styles.list} refreshing={loading} onRefresh={() => void openCustomers()} renderItem={({ item }) => <EntityRow title={item.name || '名称未設定'} subtitle={item.phone || item.mobile_phone || '電話番号未設定'} onPress={() => void openCustomer(item)} />} ListEmptyComponent={<EmptyState title="顧客がありません" description="検索条件を変えてお試しください。" />} /></ListState></Screen>;
...
function Nav({ active, onToday, onVehicles, onMaintenance, onCustomers, onQuotes }: { active: Page; onToday: () => void; onVehicles: () => void; onMaintenance: () => void; onCustomers: () => void; onQuotes: () => void }) {
  ...
  return <View style={styles.bottomTabs} accessibilityRole="tablist">{items.map((item) => <TouchableOpacity key={item.key} style={[styles.navButton, active === item.key && styles.navButtonActive]} onPress={item.onPress} accessibilityRole="tab" accessibilityLabel={item.label} accessibilityState={{ selected: active === item.key }}><Text numberOfLines={1} adjustsFontSizeToFit minimumFontScale={0.8} style={active === item.key ? styles.navTextActive : styles.navText}>{item.label}</Text></TouchableOpacity>)}</View>;
}
...
const styles = StyleSheet.create({
  ...
  errorBox: { backgroundColor: colors.errorSoft, borderRadius: 10, borderWidth: 1, borderColor: '#FECDCA', paddingHorizontal: 12, paddingVertical: 10, marginTop: 12, marginBottom: 8 }, error: { color: colors.error, fontSize: 13, lineHeight: 19 }, muted: { color: colors.muted, fontSize: 14, lineHeight: 21 }, center: { flex: 1, alignItems: 'center', justifyContent: 'center', padding: 28, backgroundColor: colors.canvas }, state: { flex: 1, alignItems: 'center', justifyContent: 'center', padding: 28 }, restoreLogo: { width: 184, height: 72, marginBottom: 28 }, empty: { alignItems: 'center', paddingHorizontal: 28, paddingTop: 68 }, emptyCompact: { paddingTop: 30, paddingBottom: 14 }, emptyTitle: { color: colors.ink, fontSize: 16, lineHeight: 22, fontWeight: '700', textAlign: 'center' }, emptyText: { color: colors.muted, fontSize: 14, lineHeight: 21, textAlign: 'center', marginTop: 6 },
