import MobileMasterSelect from './src/MobileMasterSelect';
import LoginCaptcha from './src/LoginCaptcha';
import { passwordLogin } from './src/passwordLogin';
import { useCallback, useEffect, useEffectEvent, useMemo, useRef, useState, createContext, useContext } from 'react';
...
import GarageMobileV2 from './src/v2/GarageMobileV2';
...
export function GarageMobileApp() {
  ...
  const [vehicleDraft, setVehicleDraft] = useState({
    ...
    maker: '',
    modelName: '',
    registrationNo: '', inspectionExpiryDate: '', liabilityInsuranceExpiryDate: '',
    mileageKm: '',
    color: '',
    locationName: '',
    description: '',
  ...
  function resetLocalState() {
    ...
    setStore(null); setStores([]); setPage('stores'); setVehicles([]); setVehicle(null); setToday(null); setJobs([]); setJob(null); setCustomers([]); setCustomer(null); setQuotes([]); setQuote(null); setVehicleSearch(''); setVehicleDraft({ managementNo: '', vin: '', maker: '', modelName: '', registrationNo: '', inspectionExpiryDate: '', liabilityInsuranceExpiryDate: '', mileageKm: '', color: '', locationName: '', description: '' }); setVehicleEditing(false); setVehicleEditDraft({ managementNo: '', maker: '', modelName: '', grade: '', registrationNo: '', inspectionExpiryDate: '', liabilityInsuranceExpiryDate: '', mileageKm: '', color: '', locationName: '', description: '' }); setCustomerSearch(''); setQuoteVehicle(''); setError(null); setLoading(false); setQuoteTitle(''); setItemName(''); setItemPrice(''); setQuoteValidation(null);
  }
  ...
  async function openVehicles(query = vehicleSearch) {
    ...
    if (next) { setVehicles(next.vehicles); setVehicleOffset(next.nextOffset ?? null); setVehicleQuery(query.trim()); }
  }
  function startVehicleCreate() {
    setError(null);
    retry.current = null;
    setVehicleDraft({ managementNo: '', vin: '', maker: '', modelName: '', registrationNo: '', inspectionExpiryDate: '', liabilityInsuranceExpiryDate: '', mileageKm: '', color: '', locationName: '', description: '' });
    setPage('vehicleCreate');
  }
  async function saveVehicle() {
    if (!store) return;
    const mileageText = vehicleDraft.mileageKm.trim();
    const mileageKm = mileageText ? Number(mileageText) : undefined;
    if (!vehicleDraft.vin.trim() || !vehicleDraft.maker.trim() || !vehicleDraft.modelName.trim()) {
      setError('車台番号・メーカー・車名を入力してください。');
      return;
    }
    if (mileageKm !== undefined && (!Number.isSafeInteger(mileageKm) || mileageKm < 0)) {
      setError('走行距離は0以上の整数で入力してください。');
      return;
    }
    const draft: VehicleDraft = {
      managementNo: vehicleDraft.managementNo.trim() || undefined,
      vin: vehicleDraft.vin.trim(),
      maker: vehicleDraft.maker.trim(),
      modelName: vehicleDraft.modelName.trim(),
      registrationNo: vehicleDraft.registrationNo.trim() || undefined,
      inspectionExpiryDate: vehicleDraft.inspectionExpiryDate || undefined, liabilityInsuranceExpiryDate: vehicleDraft.liabilityInsuranceExpiryDate || undefined,
      mileageKm,
      color: vehicleDraft.color.trim() || undefined,
      locationName: vehicleDraft.locationName.trim() || undefined,
      description: vehicleDraft.description.trim() || undefined,
      status: '在庫中',
    };
    retry.current = () => void saveVehicle();
    const saved = await run(() => mobileApi.createVehicle(store.id, draft), true);
    if (saved) {
      setVehicles((current) => [saved, ...current.filter((item) => item.id !== saved.id)]);
      setPage('vehicles');
    }
  }
  async function openVehicle(next: Pick<Vehicle, 'id'>) {
    if (!store) return;
    ...
    if (loaded) {
      ...
      setVehicleEditDraft({
        ...
        inspectionExpiryDate: loaded.vehicle.inspectionExpiryDate || '', liabilityInsuranceExpiryDate: loaded.vehicle.liabilityInsuranceExpiryDate || '',
  ...
  function startVehicleEdit() {
    ...
    setVehicleEditDraft({
      ...
      inspectionExpiryDate: vehicle.vehicle.inspectionExpiryDate || '', liabilityInsuranceExpiryDate: vehicle.vehicle.liabilityInsuranceExpiryDate || '',
  ...
  async function saveVehicleEdit() {
    ...
    const updated = await run(() => mobileApi.updateVehicle(store.id, vehicle.vehicle.id, {
      ...
      inspectionExpiryDate: vehicleEditDraft.inspectionExpiryDate || null, liabilityInsuranceExpiryDate: vehicleEditDraft.liabilityInsuranceExpiryDate || null,
      mileageKm,
  ...
  async function openMaintenance() {
    ...
    if (next) { setJobs(next.jobs); setMaintenanceOffset(next.nextOffset ?? null); }
  }
  ...
  async function startQuoteFromJob() {
    if (!store || !job?.customer_id) {
      setError('この整備案件には顧客が紐づいていません。');
      return;
    }
    const loaded = await run(() => mobileApi.customerDetail(store.id, job.customer_id!));
    if (!loaded) return;
    setCustomer(loaded);
    const selectedVehicleId = job.vehicle_id && loaded.vehicles.some((item) => item.id === job.vehicle_id) ? job.vehicle_id : '';
    startQuote(selectedVehicleId);
  }
  async function openCustomers(query = customerSearch) {
    ...
    if (next) { setCustomers(next.customers); setCustomerOffset(next.nextOffset ?? null); setCustomerQuery(query.trim()); }
  }
  ...
  async function openQuotes() {
    ...
    if (next) { setQuotes(next.quotes); setQuoteOffset(next.nextOffset ?? null); }
  }
  ...
  function startQuote(selectedVehicleId = '') { setError(null); retry.current = null; setQuoteVehicle(selectedVehicleId); setQuoteTitle(''); setItemName(''); setItemPrice(''); setQuoteValidation(null); setIdempotencyKey(operationKey()); setPage('quoteCreate'); }
  ...
  if (!session) return <Login email={email} password={password} error={error} loading={loading} onEmail={setEmail} onPassword={setPassword} onLogin={(captchaToken) => run(async () => { await passwordLogin(email, password, captchaToken); setPassword(''); }, true)} />;
  ...
  if (page === 'today' && store) return <GarageMobileV2 store={store} onStore={openStorePicker} onLogout={() => void logout()} />;
...
function Login({ email, password, error, loading, onEmail, onPassword, onLogin }: { email: string; password: string; error: string | null; loading: boolean; onEmail: (value: string) => void; onPassword: (value: string) => void; onLogin: (captchaToken?: string) => Promise<unknown> }) {
  const [captchaReady, setCaptchaReady] = useState(false);
  const [captchaToken, setCaptchaToken] = useState<string>();
  const [captchaAttempt, setCaptchaAttempt] = useState(0);
  const onCaptchaState = useCallback((ready: boolean, token?: string) => { setCaptchaReady(ready); setCaptchaToken(token); }, []);
  async function submit() {
    if (!captchaReady || loading) return;
    setCaptchaReady(false);
    try { await onLogin(captchaToken); } finally { setCaptchaToken(undefined); setCaptchaAttempt(value => value + 1); }
  }
  return <SafeAreaView style={styles.loginScreen}><KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}><ScrollView contentContainerStyle={styles.loginContent} keyboardShouldPersistTaps="handled"><Image source={require('./assets/garage-link-logo.png')} style={styles.logo} resizeMode="contain" accessibilityLabel="GARAGE LINK" /><Text style={styles.loginTitle}>ログイン</Text><Text style={styles.loginLead}>アカウント情報を入力してください</Text><View style={styles.loginCard}><FieldLabel label="メールアドレス" /><TextInput style={styles.input} accessibilityLabel="メールアドレス" placeholder="name@example.com" placeholderTextColor={colors.muted} autoCapitalize="none" autoCorrect={false} keyboardType="email-address" textContentType="username" autoComplete="email" value={email} onChangeText={onEmail} /><FieldLabel label="パスワード" /><TextInput style={styles.input} accessibilityLabel="パスワード" placeholder="パスワードを入力" placeholderTextColor={colors.muted} secureTextEntry textContentType="password" autoComplete="current-password" value={password} onChangeText={onPassword} /><LoginCaptcha key={captchaAttempt} attempt={captchaAttempt} onState={onCaptchaState} /><ActionButton title="ログイン" onPress={() => void submit()} disabled={!email || !password || loading || !captchaReady} />{error && <ErrorNotice message={error} />}</View></ScrollView></KeyboardAvoidingView></SafeAreaView>;
}
