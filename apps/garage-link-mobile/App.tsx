...
import { buttonLayout, createRequestCoordinator, mergePage, parentTab, quoteVehicleId, userFacingError } from './src/qualityState';
...
function TodayScreen({ today, onMaintenance, onJob, tablet }: { today: Today | null; onMaintenance: () => void; onJob: (id: string) => void; tablet: boolean }) {
  ...
  return <ScrollView contentContainerStyle={tablet ? styles.todayTablet : styles.today}>{groups.map(([title, items]) => <View key={title} style={[styles.card, tablet && styles.cardTablet]}><View style={styles.sectionHeading}><Text style={styles.section}>{title}</Text><Text style={styles.count}>{items.length > 8 && title !== '今日の予約' ? `先頭8件 / 取得${items.length}件` : `${items.length}件表示`}</Text></View>{items.length ? (title === '今日の予約' ? items : items.slice(0, 8)).map((item) => {
    const content = <><Text style={styles.todayItemTitle}>{item.job_no || item.appointment_type || item.job_type || '予定'}</Text><Text style={styles.todayItemMeta}>{statusLabel(item.status)} · {date(item.scheduled_at || item.scheduled_delivery_at || item.scheduled_in_at)}</Text>{item.assigned_user_name && <Text style={styles.todayItemMeta}>担当 {item.assigned_user_name}</Text>}</>;
    return title === '今日の予約' ? <View key={item.id} style={styles.todayItem}>{content}</View> : <TouchableOpacity key={item.id} style={styles.todayItem} onPress={() => onJob(item.id)} accessibilityRole="button" accessibilityLabel={`${item.job_no || '整備'}の詳細を開く`}>{content}</TouchableOpacity>;
