...
export const MOBILE_VEHICLE_FIELDS = 'id, management_no, vehicle_type, maker, model_name, grade, registration_no, first_registration_month, model_year, displacement_cc, mileage_km, color, inspection_expiry_date, liability_insurance_expiry_date, base_price, total_price, status, location_name, description, updated_at';
...
export const MOBILE_QUOTE_FIELDS = 'tax_display_mode, discount_input_amount, id, quote_no, title, status, issue_status, issue_date, expiry_date, customer_id, vehicle_id, deal_id, maintenance_job_id, customer_name, customer_phone, customer_email, customer_address, customer_honorific, vehicle_label, vehicle_maker, vehicle_model_name, vehicle_year, vehicle_mileage_km, vehicle_vin, vehicle_inspection_expiry_date, subtotal_amount, tax_amount, discount_amount, trade_in_amount, total_amount, customer_note, updated_at';
export const MOBILE_QUOTE_ITEM_FIELDS = 'line_discount_input_amount, part_id, cost_price, tax_category, unit, note, id, quote_id, item_order, item_type, name, description, quantity, unit_price, tax_rate, tax_amount, amount';

export function mobileVehicle(row: Record<string, unknown>) {
  return { id: row.id, managementNo: row.management_no, vehicleType: row.vehicle_type, maker: row.maker, modelName: row.model_name, grade: row.grade, registrationNo: row.registration_no, firstRegistrationMonth: row.first_registration_month, modelYear: row.model_year, displacementCc: row.displacement_cc, mileageKm: row.mileage_km, color: row.color, inspectionExpiryDate: row.inspection_expiry_date, liabilityInsuranceExpiryDate: row.liability_insurance_expiry_date, basePrice: row.base_price, totalPrice: row.total_price, status: row.status, locationName: row.location_name, description: row.description, updatedAt: row.updated_at };
}
...
export function mobileQuote(row: Record<string, unknown>, items: Array<Record<string, unknown>> = []) {
  return {
    ...
    taxDisplayMode: row.tax_display_mode, discountInputAmount: row.discount_input_amount, subtotalAmount: row.subtotal_amount, taxAmount: row.tax_amount, discountAmount: row.discount_amount,
    tradeInAmount: row.trade_in_amount, totalAmount: row.total_amount, customerNote: row.customer_note, updatedAt: row.updated_at,
    items: items.map((item) => ({ id: item.id, itemOrder: item.item_order, itemType: item.item_type, lineDiscountInputAmount:item.line_discount_input_amount,partId:item.part_id,costPrice:item.cost_price, name: item.name, description: item.description, taxCategory: item.tax_category, unit:item.unit,note:item.note, quantity: item.quantity, unitPrice: item.unit_price, taxRate: item.tax_rate, taxAmount: item.tax_amount, amount: item.amount })),
  };
}
