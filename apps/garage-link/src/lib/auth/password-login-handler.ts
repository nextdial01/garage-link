...
export async function passwordLogin(request: NextRequest, nativeSession = false) {
    ...
    cookies: {
      ...
      setAll: (cookiesToSet) => {
        for (const cookie of cookiesToSet) {
          const index = authCookies.findIndex((existing) => existing.name === cookie.name);
          if (index >= 0) authCookies[index] = cookie;
          else authCookies.push(cookie);
        }
      },
    },
  });
  ...
  const accessToken = typeof session === 'object' && 'access_token' in session ? session.access_token : null;
  const refreshToken = typeof session === 'object' && 'refresh_token' in session ? session.refresh_token : null;
  if (nativeSession && (typeof accessToken !== 'string' || typeof refreshToken !== 'string')) {
    return loginError('LOGIN_SECURITY_CHECK_FAILED', 503);
  }
  const response = NextResponse.json(nativeSession ? { ok: true, session: { access_token: accessToken, refresh_token: refreshToken } } : { ok: true }, { headers: { 'cache-control': 'no-store' } });
  if (!nativeSession) authCookies.forEach(({ name, value, options }) => {
