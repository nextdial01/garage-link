...
export default async function EmailOtpPage({ searchParams }: { searchParams?: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-slate-50 px-6 py-12 text-slate-950">
      <section className="w-full max-w-md rounded-[22px] border border-slate-200/80 bg-white p-6 shadow-[0_20px_55px_rgba(15,35,70,0.10)] sm:p-9">
        ...
        <h1 className="mt-2 text-xl font-black text-[#061735]">メールによる本人確認</h1>
        <p className="mt-2 text-sm leading-6 text-slate-600">登録済みの管理者メールへ確認コードを送信します。</p>
        <EmailOtpForm returnPath={returnPath} />
      </section>
    </main>
  );
}
