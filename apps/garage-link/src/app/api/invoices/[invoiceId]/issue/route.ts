...
export async function POST(request: NextRequest, context: { params: Promise<{ invoiceId: string }> }) {
  ...
  if (!validIdempotencyKey(body.idempotencyKey)) return NextResponse.json({ ok: false, error: '処理番号が不正です。' }, { status: 400 });
  const { data, error } = await supabase.rpc('issue_garage_invoice', { p_invoice_id: invoiceId, p_idempotency_key: body.idempotencyKey, p_correlation_id: crypto.randomUUID() });
