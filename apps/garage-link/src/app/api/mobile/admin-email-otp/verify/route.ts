...
import { getAdminEmailOtpSecret, mobileDeviceTokenHash, otpHash, randomDeviceToken } from '@/lib/security/adminEmailOtp';
...
export async function POST(request: NextRequest) {
  ...
  const [hashedCode, hashedDeviceToken] = await Promise.all([
    ...
    mobileDeviceTokenHash(deviceToken),
  ]);
  const { data, error } = await context.service.rpc('verify_admin_email_otp_mobile_device', {
  ...
  return NextResponse.json({ ok: true, trustedDeviceToken: deviceToken });
}
