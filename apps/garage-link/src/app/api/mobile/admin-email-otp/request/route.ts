...
export async function POST(request: NextRequest) {
  const context = await getAuthenticatedAdminContext(request, { allowBearer: true });
  ...
  const { data: challengeId, error } = await context.service.rpc('create_admin_email_otp_challenge', {
  ...
  if (error) {
    const limited = /otp_resend_too_soon|otp_rate_limited/.test(error.message ?? '');
    return NextResponse.json({ error: limited ? '確認コードは1分後に再送できます。' : '確認コードを作成できませんでした。', ...(limited ? { retryAfter: 60 } : {}) }, { status: limited ? 429 : 503, headers: limited ? { 'Retry-After': '60' } : undefined });
  }
