...
export default function StandaloneInvoicePreviewPage() {
  ...
  useEffect(() => {
    ...
    async function loadPreview() {
      ...
      const { data: store } = await supabase
        ...
        .select(
          'name, company_name, representative_name, invoice_registration_number, postal_code, address, building, phone, fax, email, bank_name, bank_branch_name, bank_account_type, bank_account_number, bank_account_holder, invoice_note, logo_image_path, seal_image_path, document_primary_color, document_footer_text'
        )
  ...
  const honorific = invoice?.customer_honorific ?? '様';
    ...
    <main className="min-h-screen bg-slate-100 px-4 py-8 text-slate-950 print:bg-white print:p-0">
      ...
      <div className="no-print mx-auto mb-5 flex max-w-[960px] flex-col gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Link href={`/invoices/${invoiceId}`} className="rounded-xl border border-slate-300 bg-white px-4 py-2 text-sm font-bold text-slate-700 hover:bg-slate-50">
            詳細に戻る
          </Link>
      ...
      <section className="document-paper relative mx-auto min-h-[1120px] w-[210mm] max-w-full overflow-hidden border border-slate-200 bg-white p-10 shadow-sm print:min-h-0 print:max-w-none print:p-0 print:shadow-none">
        ...
        <section className="mt-8 grid gap-8 md:grid-cols-2">
          <div className="rounded-xl border border-slate-200 p-4">
            ...
            <p className="mt-4 text-xl font-bold">{displayValue(invoice?.customer_name)} {honorific}</p>
            <p className="mt-2 text-sm">住所: {displayValue(invoice?.customer_address)}</p>
            <p className="text-sm">電話番号: {displayValue(invoice?.customer_phone)}</p>
          </div>
          <div className="rounded-xl border border-slate-200 p-4">
            ...
            <dl className="mt-3 grid grid-cols-[92px_minmax(0,1fr)] gap-x-4 gap-y-1 text-sm">
              <dt className="font-semibold text-slate-500">メーカー</dt><dd>{displayValue(invoice?.vehicle_maker)}</dd>
              <dt className="font-semibold text-slate-500">車種名</dt><dd>{displayValue(invoice?.vehicle_model_name ?? invoice?.vehicle_label)}</dd>
            </dl>
          </div>
        </section>
        ...
        <section className="mt-6 grid gap-4 md:grid-cols-2">
          ...
          <div className="rounded-lg border border-slate-300 p-4 text-sm">
            ...
            {company?.invoice_note && <p className="mt-2 whitespace-pre-wrap text-slate-600">{company.invoice_note}</p>}
            {invoice?.customer_note && <p className="mt-2 whitespace-pre-wrap text-slate-600">{invoice.customer_note}</p>}
          </div>
        </section>
