          ...
          <div className="space-y-4 border-b border-slate-200 p-5">
            <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
              ...
              <div className="grid w-full min-w-0 gap-3 sm:grid-cols-2 2xl:w-auto 2xl:grid-cols-[minmax(240px,1fr)_auto_auto_auto]">
                ...
                <select aria-label="顧客の並び順" value={sortOrder} onChange={(e) => setSortOrder(e.target.value as 'next_action' | 'recent_status')} className="rounded-xl border border-slate-300 px-4 py-2.5 text-sm outline-none transition focus:border-blue-600 focus:ring-4 focus:ring-blue-100"><option value="next_action">次回対応が近い順</option><option value="recent_status">ステータス順</option></select>
                <select aria-label="顧客ステータス" value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="rounded-xl border border-slate-300 px-4 py-2.5 text-sm outline-none transition focus:border-blue-600 focus:ring-4 focus:ring-blue-100"><option value="">すべての状態</option>{statusOptions.map((status) => <option key={status}>{status}</option>)}</select>
                <select aria-label="次回対応日" value={actionFilter} onChange={(e) => setActionFilter(e.target.value as 'all' | 'today' | 'overdue')} className="rounded-xl border border-slate-300 px-4 py-2.5 text-sm outline-none transition focus:border-blue-600 focus:ring-4 focus:ring-blue-100"><option value="all">すべての次回対応</option><option value="today">今日対応</option><option value="overdue">期限超過</option></select>
              </div>
            </div>
          </div>
