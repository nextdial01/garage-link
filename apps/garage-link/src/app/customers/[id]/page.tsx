...
type DealRow = { id: string; vehicle_id: string | null; deal_no: string | null; title: string | null; status: string | null; next_action_at: string | null };
type VehicleRow = { id: string; maker: string | null; model_name: string | null; management_no: string | null };
type MaintenanceRow = { id: string; vehicle_id: string | null; job_no: string | null; job_type: string | null; status: string | null };
  ...
  useEffect(() => {
    async function loadCustomer() {
      try {
        ...
        const [customerResult, dealResult, vehicleResult, maintenanceResult, logResult] = await Promise.all([
          supabase.from<CustomerRow>('customers').select('*').eq('id', customerId).eq('store_id', member.store_id).single(),
          supabase.from<DealRow>('deals').select('id, vehicle_id, deal_no, title, status, next_action_at').eq('customer_id', customerId).eq('store_id', member.store_id).order('created_at', { ascending: false }),
          supabase.from<VehicleRow>('vehicles').select('id, maker, model_name, management_no').eq('store_id', member.store_id),
          supabase.from<MaintenanceRow>('maintenance_jobs').select('id, vehicle_id, job_no, job_type, status').eq('customer_id', customerId).eq('store_id', member.store_id).order('created_at', { ascending: false }),
    ...
    <AppShell activeLabel="顧客管理" title="顧客詳細" description="顧客情報・LINE・関連商談を確認、編集します" actionButton={<div className="flex flex-wrap gap-2"><button type="button" onClick={() => void handleSave()} disabled={isSaving} className="rounded-xl bg-blue-600 px-4 py-2 text-sm font-bold text-white shadow-sm hover:bg-blue-700 disabled:opacity-60">{isSaving ? '保存中...' : '保存する'}</button><SoftDeleteButton tableName="customers" rowId={customerId} storeId={storeId} targetType="customer" targetLabel={form.name || '顧客'} redirectHref="/customers" /><Link href="/customers" className="rounded-xl border border-slate-300 bg-white px-4 py-2 text-sm font-bold text-slate-700 shadow-sm hover:bg-slate-50">顧客一覧に戻る</Link></div>}>
      {isLoading ? <p className="rounded-2xl bg-white p-5 text-sm text-slate-500 shadow-sm">読み込み中...</p> : notFound ? <p className="rounded-2xl bg-white p-5 text-sm font-semibold text-slate-500 shadow-sm">顧客が見つかりません。</p> : (
        <div className="mx-auto max-w-7xl space-y-6">
          ...
          <Section title="関連商談"><div className="overflow-x-auto"><table className="w-full min-w-[760px] text-left text-sm"><thead className="bg-slate-50 text-xs font-bold text-slate-500"><tr>{['商談番号','商談タイトル','対象車両','ステータス','次回対応日','詳細'].map((h)=><th key={h} className="px-4 py-3">{h}</th>)}</tr></thead><tbody className="divide-y divide-slate-100">{deals.length === 0 ? <tr><td colSpan={6} className="px-4 py-4 text-slate-500">データがありません</td></tr> : deals.map((deal)=><tr key={deal.id}><td className="px-4 py-3">{displayValue(deal.deal_no)}</td><td className="px-4 py-3 font-semibold">{displayValue(deal.title)}</td><td className="px-4 py-3">{vehicleLabel(vehicleMap.get(deal.vehicle_id ?? ''))}</td><td className="px-4 py-3">{displayValue(deal.status)}</td><td className="px-4 py-3">{formatDateTime(deal.next_action_at)}</td><td className="px-4 py-3"><Link href={`/deals/${deal.id}`} className="font-bold text-blue-700 hover:underline">詳細</Link></td></tr>)}</tbody></table></div></Section>
