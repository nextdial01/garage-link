...
export default function CustomerDetailPage() {
  ...
  const activityTimeline = useMemo(() => [
    ...deals.map((item) => ({ date: item.last_contact_at ?? item.next_action_at, type: '商談', label: item.title ?? item.deal_no ?? '商談', detail: item.status ?? '未設定', href: `/deals/${item.id}` })),
    ...maintenance.map((item) => ({ date: item.actual_delivery_date, type: '整備', label: item.job_type ?? item.job_no ?? '整備', detail: item.status ?? '未設定', href: `/maintenance/${item.id}` })),
    ...quotes.map((item) => ({ date: item.created_at, type: '見積', label: item.title ?? item.quote_no ?? '見積', detail: item.status ?? '未設定', href: `/quotes/${item.id}` })),
    ...invoices.map((item) => ({ date: item.created_at, type: '請求', label: item.title ?? item.invoice_no ?? '請求', detail: item.status ?? '未設定', href: `/invoices/${item.id}` })),
    ...lineLogs.map((item) => ({ date: item.sent_at ?? item.created_at, type: 'LINE', label: item.title ?? item.message_type ?? 'LINE送信', detail: item.send_status ?? '未設定', href: '/line/message-logs' })),
  ].filter((item) => item.date).sort((a, b) => String(b.date).localeCompare(String(a.date))).slice(0, 12), [deals, maintenance, quotes, invoices, lineLogs]);
    ...
    <AppShell activeLabel="顧客管理" title="顧客詳細" description="顧客情報・LINE・関連商談を確認、編集します" actionButton={<div className="flex flex-wrap gap-2"><button type="button" onClick={() => void handleSave()} disabled={isSaving} className="rounded-xl bg-blue-600 px-4 py-2 text-sm font-bold text-white shadow-sm hover:bg-blue-700 disabled:opacity-60">{isSaving ? '保存中...' : '保存する'}</button><SoftDeleteButton tableName="customers" rowId={customerId} storeId={storeId} targetType="customer" targetLabel={form.name || '顧客'} redirectHref="/customers" /><Link href="/customers" className="rounded-xl border border-slate-300 bg-white px-4 py-2 text-sm font-bold text-slate-700 shadow-sm hover:bg-slate-50">顧客一覧に戻る</Link></div>}>
      {isLoading ? <p className="rounded-2xl bg-white p-5 text-sm text-slate-500 shadow-sm">読み込み中...</p> : notFound ? <p className="rounded-2xl bg-white p-5 text-sm font-semibold text-slate-500 shadow-sm">顧客が見つかりません。</p> : (
        <div className="mx-auto max-w-7xl space-y-6">
          ...
          <Section title="活動履歴"><div className="space-y-3">{activityTimeline.length === 0 ? <p className="text-sm text-slate-500">まだ履歴がありません。</p> : activityTimeline.map((item, index) => <Link key={`${item.type}-${item.href}-${index}`} href={item.href} className="flex items-center gap-3 rounded-xl border border-slate-200 px-4 py-3 transition hover:bg-blue-50"><span className="w-12 shrink-0 text-xs font-bold text-slate-400">{item.type}</span><span className="min-w-0 flex-1"><span className="block truncate text-sm font-bold text-slate-800">{item.label}</span><span className="mt-1 block text-xs text-slate-500">{item.detail} / {formatDateTime(item.date)}</span></span><span className="text-blue-700">→</span></Link>)}</div></Section>
