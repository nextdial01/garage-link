...
test('Manual Gmail Bridge creates a plus address, redacts checkpoints, and polls Auth confirmation',async()=>{
  ...
  const session=createManualGmailSession('Owner.Name@gmail.com',marker);
  assert.equal(session.emailAddress,'owner.name+garage-link-12345678@gmail.com');
  ...
  assert.throws(()=>createManualGmailSession('not-gmail@example.com',marker),/MANUAL_GMAIL_PLUS_ADDRESS_UNAVAILABLE/);
});
