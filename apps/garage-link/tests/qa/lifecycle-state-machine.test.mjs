...
import { assertProvenance,assertRunId,assertSafeContract,classifyFailure,fixtureIdentity,nextAfter,removeCheckpoint,resolveKeychainAccount,transitionAllowed,validateServiceKey,writeCheckpoint } from '../../scripts/qa/lifecycle-core.mjs';
...
test('abort is terminal and keychain account fails closed',()=>{assert.equal(transitionAllowed('ABORTED_CLEAN','CREATED'),false);assert.equal(resolveKeychainAccount({USER:'operator'}),'operator');assert.throws(()=>resolveKeychainAccount({}),/KEYCHAIN_ACCOUNT_UNAVAILABLE/)})
