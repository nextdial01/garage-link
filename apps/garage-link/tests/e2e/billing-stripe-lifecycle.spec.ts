...
test.describe.serial('GARAGE LINK Stripe commercial checkpoints', () => {
  ...
  test('7 downgrade', async ({ page }) => {
    ...
    if (currentPlan !== 'starter') {
      ...
      expectAccepted(response);
      // Unlike upgrades, a downgrade only *schedules* the plan change - but
      // /api/billing/change-plan writes pending_plan/pending_plan_effective_at
      // synchronously inside the same request that returns this response (it reads
      // current_period_end and writes the schedule before responding), so by the time
      // expectAccepted() returns, the schedule is already committed. billing_sync_operations
      // only reaches 'completed' later, via whatever webhook happens to arrive next for
      // this subscription (observed anywhere from ~2s to ~100s later) - it is not a signal
      // that the schedule write happened, so waiting on it here was both unnecessary and
      // unreliably slow. Verify the schedule directly instead.
      await waitFor(async () => (await readDbSubscription()).data?.pending_plan === 'starter',
        'downgrade scheduled (pending_plan written)');
      console.info(`[checkpoint:7] pending_plan_effective_at=${(await readDbSubscription()).data?.pending_plan_effective_at}`);
      await advanceBillingPeriod();
