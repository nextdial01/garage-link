-- Deployable migration promotion of supabase/schema/040_followup_candidate_generation.sql.
-- See 20260630000000_delivery_candidate_event_types.sql for why this promotion exists.
--
-- Dependencies (must apply after both, hence its position at 20260701000000):
--   - 20260630000000 (037): widens inspection_reminder_events.event_type CHECK to allow
--     periodic_inspection / post_delivery_follow_up / review_request / long_no_contact.
--     Without it, the INSERTs below violate the CHECK constraint.
--   - 20260630000200 (039): adds customers.last_contact_at, read by the long_no_contact block.
--     Without it, this function fails at execution time with "column does not exist".
--
-- generate_followup_candidate_events already includes the auth.uid()/store_members
-- authorization check added by 20260703000000_generate_events_store_authorization.sql (that
-- migration was written after schema/040 already had it, so re-applying 20260703 after this file
-- is a harmless, byte-identical no-op for this function — it remains necessary for the OTHER
-- function it patches, generate_inspection_reminder_events in schema/035, which is not part of
-- this 037-041 promotion).
--
-- The SQL body below is byte-identical to supabase/schema/040_followup_candidate_generation.sql
