-- Scope get_inspection_reminder_eligibility_summary's event_counts to event_type='inspection_reminder'.
--
-- Why: 040 (generate_followup_candidate_events, wired up in the same release as this migration)
-- writes rows into the same inspection_reminder_events table with other event_type values
-- (periodic_inspection / post_delivery_follow_up / review_request / long_no_contact). Without this
-- scope, the 車検案内設定 (inspection-reminder-only) settings screen's "案内イベント（累計）" counts
-- would silently include unrelated follow-up candidate counts, misleading owner/admin operators
-- diagnosing car-inspection reminder delivery specifically.
--
-- Everything else in the function is unchanged (auth checks, vehicle eligibility counts, PII
-- minimization). Only the event_counts CTE gains one additional filter.
...
begin
  ...
  -- ── Event counts by status (車検案内のみ) ─────────────────────────────────────────────
  -- event_type を限定しないと、040 (generate_followup_candidate_events) が生成する
  -- periodic_inspection 等の件数が車検案内の集計に混ざってしまうため明示的に絞る。
  event_counts as (
...
-- Revoke default PUBLIC access before granting to authenticated only (unchanged from 20260627).
...
-- Rollback (manual, if needed): re-run 20260627000000_inspection_reminder_eligibility_fn.sql
-- to restore the unscoped event_counts (all event_types).
