-- Header/items are a single RLS-preserving transaction. Existing accounting guards stay active.
begin;
alter table public.quote_items alter column unit_price type numeric, alter column cost_price type numeric;
alter table public.invoice_items alter column unit_price type numeric, alter column cost_price type numeric;
...
begin
 ...
 for v_line in select value from jsonb_array_elements(p_items) loop
  if jsonb_typeof(v_line)<>'object' or exists(select 1 from jsonb_object_keys(v_line) k where not(k=any(array['item_type','name','description','part_id','cost_price','unit','note','quantity','unit_price','tax_rate','tax_category','amount','tax_amount']))) then raise exception 'invalid_document_item_fields' using errcode='22023'; end if;
...
begin
  ...
  for v_item in select value from jsonb_array_elements(p_items) loop
    ...
    insert into public.quote_items (
      store_id,quote_id,item_order,item_type,name,description,quantity,unit_price,tax_rate,tax_amount,amount,unit,note,tax_category,part_id,cost_price
    ) values (
      ...
      (v_item->>'tax_rate')::numeric,(v_item->>'tax_amount')::integer,(v_item->>'amount')::integer,v_item->>'unit',v_item->>'note',v_item->>'tax_category',nullif(v_item->>'part_id','')::uuid,nullif(v_item->>'cost_price','')::numeric
    );
  end loop;
...
begin
  ...
  insert into public.invoice_items(store_id,invoice_id,item_order,item_type,name,description,quantity,unit_price,tax_rate,tax_amount,amount,unit,note,tax_category,part_id,cost_price)
    select p_store_id,p_invoice_id,item_order,item_type,name,description,quantity,
      unit_price,tax_rate,round(tax_amount)::integer,round(amount)::integer,unit,note,tax_category,part_id,cost_price
