...
| OPS-004 | Medium | P0 | Operation Risk | remote staging readiness | 専用remote staging Supabaseがなく、Vercel PreviewはProductionとSupabase URL・anon key・service roleを共有。メール送信credentialもPreviewへ配布されている | `35-staging-deploy-and-runtime-validation-report.md`、`evidence/staging-runtime-validation.json`。空の`KANNAGI Staging` orgは確認できたが、管理者のFree project上限2件で新規project作成が無効。deploy／Current変更／外部送信0件 | PreviewがCurrent DBへ接続し、本番相当外部送信を起動する危険がある | `KANNAGI Staging` orgでproject枠を確保し、専用staging、Preview専用credential、backup/restore、外部deny、架空fixtureを固定 | **Confirmed / capacity BLOCKED / staging未実施** |
