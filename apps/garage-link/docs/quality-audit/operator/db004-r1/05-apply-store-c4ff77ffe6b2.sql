...
do $db004_r1$
declare
  c_store_fp constant text := 'c4ff77ffe6b2';
  c_owner_fp constant text := 'e9d226b2af53';
  c_legacy_role constant text := 'staff';
  c_expected_store_status constant text := 'active';
  c_correlation constant text := '820a4de6-15fe-43ae-9779-a87e79998ddf';
  c_recovery_key_fp constant text := 'eecf0a3ad617';
...
select
  ...
  (select count(*) from public.audit_logs a where a.action='db004_independent_tenant_recovery' and a.metadata->>'correlationId'='820a4de6-15fe-43ae-9779-a87e79998ddf' and a.metadata->>'storeFingerprint'='c4ff77ffe6b2') as recovery_audit_count
from public.stores s
where left(encode(extensions.digest(s.id::text,'sha256'),'hex'),12)='c4ff77ffe6b2';
