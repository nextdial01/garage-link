...
do $db004_r1$
declare
  c_store_fp constant text := '2c127637ea1c';
  c_owner_fp constant text := '4a0360984dc0';
  c_legacy_role constant text := 'owner';
  c_expected_store_status constant text := 'trial';
  c_correlation constant text := '820a4de6-15fe-43ae-9779-a87e79998ddf';
  c_recovery_key_fp constant text := '079dd4c23908';
...
select
  ...
  (select count(*) from public.audit_logs a where a.action='db004_independent_tenant_recovery' and a.metadata->>'correlationId'='820a4de6-15fe-43ae-9779-a87e79998ddf' and a.metadata->>'storeFingerprint'='2c127637ea1c') as recovery_audit_count
from public.stores s
where left(encode(extensions.digest(s.id::text,'sha256'),'hex'),12)='2c127637ea1c';
