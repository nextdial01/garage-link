...
do $db004_r1$
declare
  c_store_fp constant text := 'eb2bcbf525ea';
  c_owner_fp constant text := 'dc5e4dbb6ea5';
  ...
  c_recovery_key_fp constant text := 'dbe342055dc9';
...
select
  ...
  (select count(*) from public.audit_logs a where a.action='db004_independent_tenant_recovery' and a.metadata->>'correlationId'='820a4de6-15fe-43ae-9779-a87e79998ddf' and a.metadata->>'storeFingerprint'='eb2bcbf525ea') as recovery_audit_count
from public.stores s
where left(encode(extensions.digest(s.id::text,'sha256'),'hex'),12)='eb2bcbf525ea';
