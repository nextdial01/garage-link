...
export async function applyStagingPasswordMinimum(token,fetchImpl=fetch){
  ...
  const response=await fetchImpl(endpoint,{method:'PATCH',headers:{authorization:`Bearer ${token}`,'content-type':'application/json'},body:JSON.stringify({password_min_length:8}),redirect:'manual',cache:'no-store'});
  ...
  if(minimum!==8)throw new Error('STAGING_PASSWORD_MINIMUM_READBACK_FAILED');
  return {project_ref:STAGING_REF,password_minimum:minimum};
}

async function main(){
  const token=required('GARAGE_STAGING_SUPABASE_MANAGEMENT_TOKEN');
  const result=await applyStagingPasswordMinimum(token);
  process.stdout.write(`${JSON.stringify({state:'STAGING_PASSWORD_MINIMUM_8_READY',...result,production_ref:PRODUCTION_REF,production_write:false})}\n`);
}
