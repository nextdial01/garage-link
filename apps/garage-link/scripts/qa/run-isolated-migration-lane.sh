...
MANIFEST="$APP_ROOT/supabase/qa/manifest.json"
QA_COUNT="$(node -e "const m=require(process.argv[1]); process.stdout.write(String(m.entries.length))" "$MANIFEST")"
cleanup(){ docker rm -f "$NAME" >/dev/null 2>&1 || true; }
trap cleanup EXIT INT TERM
echo "[qa-lane] isolated Docker apply ${QA_COUNT}/${QA_COUNT}"
...
echo "[qa-lane] rollback ${QA_COUNT}/${QA_COUNT}"
...
echo "[qa-lane] reapply ${QA_COUNT}/${QA_COUNT}"
node "$APP_ROOT/scripts/db/migration-runner.mjs" apply --container "$NAME" --environment g0b-qa --manifest "$APP_ROOT/supabase/qa/manifest.json"
echo '[qa-lane] PASS'
